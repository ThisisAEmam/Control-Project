...
% Last Modified by GUIDE v2.5 09-Jun-2020 23:23:30
...
function [sys,name] = get_system(handles)
...
x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/_Bm(z)//Am(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Bm) ']'], 'denominator',['[' num2str(Am) ']'], 'sampletime',str_Ts);
x=x+195;%tracking width
add_line(name, 'Reference/1', '_Bm(z)//Am(z)/1');

x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/_T(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(T) ']'], 'denominator','[1]', 'sampletime',str_Ts);
x=x+195;%T width
add_line(name, '_Bm(z)//Am(z)/1', '_T(z)/1');

x=x+25;%spacing
add_block('simulink/Math Operations/Sum', [name '/_Sum1'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','|+-');
x=x+20;%sum width
add_line(name, '_T(z)/1', '_Sum1/1');
%{
x=x+20;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/_1//Hs(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(Hs) ']'], 'sampletime',str_Ts);
x=x+195;%Hs width
%}
%add_line(name, '_Sum1/1', '_1//Hs(z)/1');

x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/_1//S(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(S) ']'], 'sampletime',str_Ts);
x=x+195;%S width
%add_line(name, '_1//Hs(z)/1', '_1//S(z)/1');
add_line(name, '_Sum1/1', '_1//S(z)/1');
...
if get(handles.checkbox1, 'value')%continuous plant
    ...
    x=x+35;%ZOH width
    add_line(name, '_1//S(z)/1', 'Zero-Order Hold/1');
    ...
    x=x+195;%plant width
    add_line(name, '_1//S(z)/1', 'Bp(z)//Ap(z)/1');
...
block_R=add_block('simulink/Discrete/Discrete Filter', [name '/_R(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(R) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');
%all=get(get_param(block_R, 'handle'))
x=x-195;%R width
add_line(name, 'Sum2/1', '_R(z)/1');
%{
x=x-35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/_Hr(z))'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Hr) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');
x=x-195;
%}
%add_line(name, '_R(z)/1', '_Hr(z))/1');
%add_line(name, '_Hr(z))/1)', '_Sum1/2');
add_line(name, '_R(z)/1)', '_Sum1/2');
...
[sys, name] = get_system(handles); %getting the whole model

open_system(sys); % open simulink model
...
%% generate c code button
% author: abdelrhman kadry anwar
% MATLAB function for c code generation implemeted on 09-Jun-2020 

[sys, name] = get_system(handles);%getting the whole model

%=========== Creating subsystem =================
 blocks = find_system(sys, 'SearchDepth', 1, 'Regexp', 'on','Name','^_');
  bh = [];
...
%=========== Generating code for subsystem =================
cs = configruation();
attachConfigSet(sys,cs);
setActiveConfigSet(sys, 'QuickStart_50020_6_9_22_1_0295');
get_param(name,'SolverType');
set_param(name,'SolverType','Fixed-step');
rtwbuild(strcat(name,'/Subsystem'));
%===========================================================


% --- Executes on button press in pushbutton5.
function pushbutton5_Callback(hObject, eventdata, handles)%generate .h files

% hObject    handle to pushbutton5 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

%% generate .h files button
% author: AbdelAziz Hosney AbdelAziz
% MATLAB function for .h files generation implemeted on 12-Jun-2020 
...
size_Bm= length(Bm);
size_Am= length(Am);
%============= Printing RST.h and RST.c Files===================

fileID = fopen('RST.h','a+');% .h file

fprintf(fileID, '#ifndef _RST_H_\n');

fprintf(fileID, '#define _RST_H_\n');
%=============================================
fprintf(fileID, 'extern const float R['); %R

fprintf(fileID, num2str(size_R));

fprintf(fileID, '];\n');
%=============================================
fprintf(fileID, 'extern const float S['); %S

fprintf(fileID, num2str(size_S));

fprintf(fileID, '];\n');
%=============================================
fprintf(fileID, 'extern const float T['); %T
...
fprintf(fileID2, [num2str(T(k)) ',\n']);

end

fprintf(fileID2, '};\n');

%====Printing controlled process model and closed loop desired performance Files=====

fileID3 = fopen('Process_Parameters.h','a+');% .h file

fprintf(fileID3, '#ifndef _PROCESS_PARAMETERS_H_\n');

fprintf(fileID3, '#define _PROCESS_PARAMETERS_H_\n');
%=============================================
fprintf(fileID3, 'extern const float Ts;\n'); %Ts

%=============================================
fprintf(fileID3, 'extern const float Bp['); %Bp

fprintf(fileID3, num2str(size_Bp));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float Ap['); %Ap

fprintf(fileID3, num2str(size_Ap));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float Hr['); %Hr

fprintf(fileID3, num2str(size_Hr));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float Hs['); %Hs

fprintf(fileID3, num2str(size_Hs));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float P['); %P

fprintf(fileID3, num2str(size_P));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float Bm['); %Bm

fprintf(fileID3, num2str(size_Bm));

fprintf(fileID3, '];\n');
%=============================================
fprintf(fileID3, 'extern const float Am['); %Am

fprintf(fileID3, num2str(size_Am));
...
fileID4 = fopen('Process_Parameters.cpp','a+');

fprintf(fileID4, '#include\"Process_Parameters.h\"\n');
%=============================================

fprintf(fileID4, 'const float Ts='); %Ts

fprintf(fileID4, [num2str(Ts) ';\n']);

%=============================================
fprintf(fileID4, 'const float Bp[]=\n'); %Bp
...
fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float Ap[]=\n'); %Ap
...
fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float Hr[]=\n'); %Hr

fprintf(fileID4, '{\n');

for k=1:size_Hr

fprintf(fileID4, [num2str(Hr(k)) ',\n']);

end

fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float Hs[]=\n'); %Hs

fprintf(fileID4, '{\n');

for k=1:size_Hs

fprintf(fileID4, [num2str(Hs(k)) ',\n']);

end

fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float P[]=\n'); %P

fprintf(fileID4, '{\n');

for k=1:size_P

fprintf(fileID4, [num2str(P(k)) ',\n']);

end

fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float Bm[]=\n'); %Bm

fprintf(fileID4, '{\n');

for k=1:size_Bm

fprintf(fileID4, [num2str(Bm(k)) ',\n']);

end

fprintf(fileID4, '};\n');
%===============================================
fprintf(fileID4, 'const float Am[]=\n'); %Am

fprintf(fileID4, '{\n');

for k=1:size_Am

fprintf(fileID4, [num2str(Am(k)) ',\n']);
